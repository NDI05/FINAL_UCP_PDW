<section class="border-b border-[#333] px-6 lg:px-12 py-10 max-w-3xl">
    <div class="mb-8">
        ...
        <h1 class="text-xl sm:text-2xl font-bold text-white tracking-tight mt-2">Tambah <span class="text-[#CCFF00]">Layanan</span></h1>
    </div>
    ...
    <form method="POST" action="/admin/services/create" enctype="multipart/form-data" class="space-y-6">
        <div>
        ...
        <div>
            ...
            <textarea id="description"
                      ...
                      rows="6"
                      required
                      class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono outline-none focus:border-[#CCFF00] resize-y"><?= htmlspecialchars($description ?? '') ?></textarea>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                <label for="image" class="block text-[10px] font-mono text-[#666] tracking-[.2em] uppercase mb-2">[ UPLOAD IMAGE ]</label>
                <input type="file"
                       id="image"
                       name="image"
                       accept="image/*"
