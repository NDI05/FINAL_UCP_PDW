<div class="max-w-4xl mx-auto px-6 lg:px-10 py-10">
    <div class="mb-10">
        ...
        <h1 class="text-2xl lg:text-3xl font-bold text-white tracking-tight mt-1">EDIT SERVICE</h1>
    </div>
    ...
    <form method="POST" action="/admin/services/edit?id=<?= (int)$service['id'] ?>" enctype="multipart/form-data" class="space-y-8">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="8" required
                      class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono leading-relaxed outline-none focus:border-[#CCFF00] resize-vertical rounded-none"><?= htmlspecialchars($service['description']) ?></textarea>
        </div>
        ...
        <div>
            ...
            <input type="number" id="order_num" name="order_num" value="<?= (int)$service['order_num'] ?>" min="0"
                   class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono outline-none focus:border-[#CCFF00] rounded-none">
        </div>
