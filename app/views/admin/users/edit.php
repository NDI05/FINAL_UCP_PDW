<div class="max-w-4xl mx-auto px-6 lg:px-10 py-10">
    <div class="mb-10">
        ...
        <h1 class="text-2xl lg:text-3xl font-bold text-white tracking-tight mt-1">EDIT USER</h1>
    </div>
    ...
    <form method="POST" action="<?= baseUrl('/admin/users/edit?id=' . (int)$user['id']) ?>" class="space-y-8">
        <div>
        ...
        <div>
            ...
            <input type="password" id="password" name="password"
                   class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono outline-none focus:border-[#CCFF00] rounded-none">
            <p class="mt-2 text-[10px] font-mono text-[#333]">Leave blank to keep current password</p>
        </div>
        ...
        <div>
            ...
            <select id="role" name="role"
                    class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono outline-none focus:border-[#CCFF00] rounded-none">
                <option value="editor" <?= $user['role'] === 'editor' ? 'selected' : '' ?> class="bg-[#0a0a0a]">EDITOR</option>
                <option value="admin" <?= $user['role'] === 'admin' ? 'selected' : '' ?> class="bg-[#0a0a0a]">ADMIN</option>
            </select>
        </div>
