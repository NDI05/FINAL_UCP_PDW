<div class="max-w-4xl mx-auto px-6 lg:px-10 py-10">
    <div class="mb-10">
        ...
        <h1 class="text-2xl lg:text-3xl font-bold text-white tracking-tight mt-1">EDIT ARTICLE</h1>
    </div>
    ...
    <form method="POST" action="/admin/articles/edit?id=<?= (int)$article['id'] ?>" enctype="multipart/form-data" class="space-y-8">
        <div>
            ...
            <input type="text" id="title" name="title" required value="<?= htmlspecialchars($article['title']) ?>"
                   ...
                   oninput="document.getElementById('slug').value = this.value.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'') || 'untitled'">
        </div>

        <div>
            <label for="slug" class="block text-[10px] font-mono text-[#666] tracking-[.2em] uppercase mb-2">[ SLUG ]</label>
            <input type="text" id="slug" name="slug" value="<?= htmlspecialchars($article['slug']) ?>"
                   class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono outline-none focus:border-[#CCFF00] rounded-none">
        </div>
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="16" required
                      class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono leading-relaxed outline-none focus:border-[#CCFF00] resize-vertical rounded-none"><?= htmlspecialchars($article['content']) ?></textarea>
        </div>
        ...
        <div>
            ...
            <?php if (!empty($article['image'])): ?>
                <div class="border border-[#333] p-2 inline-block">
                    <img src="/<?= htmlspecialchars($article['image']) ?>" alt="Current" class="h-32 w-auto grayscale">
                </div>
            <?php else: ?>
        ...
        <div>
            ...
            <select id="status" name="status"
                    class="w-full bg-transparent border border-[#333] px-4 py-3 text-white text-sm font-mono outline-none focus:border-[#CCFF00] rounded-none">
                <option value="draft" <?= $article['status'] === 'draft' ? 'selected' : '' ?> class="bg-[#0a0a0a]">DRAFT</option>
                <option value="published" <?= $article['status'] === 'published' ? 'selected' : '' ?> class="bg-[#0a0a0a]">PUBLISHED</option>
                <option value="archived" <?= $article['status'] === 'archived' ? 'selected' : '' ?> class="bg-[#0a0a0a]">ARCHIVED</option>
            </select>
        </div>
        ...
        <div class="flex items-center gap-4 pt-4">
            ...
            <a href="/admin/articles" class="px-8 py-3 border border-[#333] text-[10px] font-mono text-[#666] hover:text-white no-underline tracking-[.2em] uppercase">[ CANCEL ]</a>
        </div>
    </form>
</div>
