<section class="border-b border-[#333] px-6 lg:px-12 py-10 max-w-3xl">
    ...
    <?php if (!empty($article['image'])): ?>
        <div class="mt-6 border border-[#333] p-2 inline-block">
            <img src="<?= baseUrl('/' . htmlspecialchars($article['image'])) ?>" alt="<?= htmlspecialchars($article['title']) ?>" class="h-48 sm:h-64 object-contain grayscale">
        </div>
    <?php endif; ?>
    ...
    <div class="flex items-center gap-4 mt-10 pt-6 border-t border-[#333]">
        <a href="<?= baseUrl('/admin/articles/edit?id=' . $article['id']) ?>" class="px-6 py-3 bg-[#CCFF00] text-[#0a0a0a] font-bold text-xs tracking-[.2em] uppercase font-mono hover:bg-white no-underline">EDIT</a>
